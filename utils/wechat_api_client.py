...
class WechatApiClient:
    ...
    def __init__(self, app_id: str = None, app_secret: str = None, proxy: str = None, auth_manager = None):
        """
        初始化微信API客户端
        
        Args:
            app_id: 微信公众号AppID（可选，如果提供 auth_manager 则从 auth_manager 获取）
            app_secret: 微信公众号AppSecret（可选，如果提供 auth_manager 则从 auth_manager 获取）
            proxy: 代理地址，默认为None
        ...
        if auth_manager:
            ...
            if config:
                ...
                raise ValueError('auth_manager 中没有配置信息')
        else:
            if not app_id or not app_secret:
                raise ValueError('必须提供 app_id 和 app_secret，或提供 auth_manager')
            self.app_id = app_id
    ...
    @classmethod
    def from_auth_manager(cls, auth_manager, proxy: str = None):
        """
        从认证管理器创建 API 客户端
        ...
        Returns:
            WechatApiClient 实例
        """
        return cls(auth_manager=auth_manager, proxy=proxy)
    
    def _get_access_token_sync(self, force_refresh: bool = False) -> str:
        """同步方式获取访问令牌（内部方法）"""
        # 检查是否需要刷新令牌
    ...
    async def get_access_token(self, force_refresh: bool = False) -> str:
        ...
        Returns:
            str: 访问令牌
        """
        # 如果使用 auth_manager，使用它的方法
        if self.auth_manager:
            token_info = await self.auth_manager.get_access_token(force_refresh)
            self.access_token = token_info['accessToken']
            self.token_expire_time = token_info['expiresAt'] / 1000  # 转换为秒
            return self.access_token
    ...
    def _request(self, method: str, endpoint: str, params: Dict = None, data: Any = None, 
        ...
        try:
            ...
            if need_token:
                if self.auth_manager:
                    # 使用 auth_manager 获取 token（同步方式，因为 _request 是同步方法）
                    import asyncio
                    try:
                        # 尝试获取当前事件循环
                        ...
                        if loop.is_running():
                            ...
                            with concurrent.futures.ThreadPoolExecutor() as executor:
                                future = executor.submit(
                                    lambda: asyncio.run(self.auth_manager.get_access_token())
                                )
                                token_info = future.result()
                                token = token_info['accessToken']
                        else:
                            token_info = loop.run_until_complete(self.auth_manager.get_access_token())
                            token = token_info['accessToken']
                    except RuntimeError:
                        # 没有事件循环，创建一个新的
                        token_info = asyncio.run(self.auth_manager.get_access_token())
                        token = token_info['accessToken']
                else:
                    # 同步方式获取 token
                    import asyncio
